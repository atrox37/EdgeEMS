...
describe('useTableData', () => {
  ...
  it('should fetch table data successfully', async () => {
    ...
    vi.mocked(defaultRequest.get).mockResolvedValue(mockResponse)
    ...
    expect(defaultRequest.get).toHaveBeenCalledWith('/test/list', expect.any(Object))
  })
  ...
  it('should handle search data', async () => {
    ...
    expect(defaultRequest.get).toHaveBeenCalledWith(
      '/test/list',
  ...
  it('should handle delete row', async () => {
    ...
    const { defaultRequest } = await import('@/utils/request')
    vi.mocked(defaultRequest.delete).mockResolvedValue(mockResponse)

    const { deleteRow } = useTableData(mockConfig)
    const result = await deleteRow('1')

    expect(result).toBe(true)
    expect(defaultRequest.delete).toHaveBeenCalledWith('/test/delete/1')
  })
