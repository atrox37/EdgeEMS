    ...
    <div class="devices-pv__tables">
      ...
      <div class="devices-pv__table devices-pv__table--right">
        <el-table :data="rightTableData" table-layout="fixed" height="100%">
          <el-table-column prop="name" label="Name" />
          <el-table-column prop="status" label="Status" :formatter="statusFormatter" />
          <el-table-column prop="updateTime" label="Update Time" />
        </el-table>
      </div>
    </div>
...
// 状态格式化函数
const statusFormatter = (_row: any, _column: any, cellValue: number) => {
  if (cellValue === 1) return 'On'
  if (cellValue === 0) return 'Off'
  return cellValue
}
...
const updateData = () => {
  ...
  leftTableData.value.forEach((item, index) => {
    // 为每个数据项添加一些随机变量
    const randomChange = (Math.random() - 0.5) * 0.1 // ±5% 的随机变量
